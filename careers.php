...
<div class="loader" style="display: none;">
                    <img src="images/preloader-eod.gif" alt="Easy Outdesk">
                </div>
                                    ...
                                    <h6 class="my-title"></h6>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <input type="text" id="accomplishment-id" name="accomplishment-id" class="mandatory form-control">
...
$(document).ready(function() {
    ...
    $("body").delegate("span.add-more-items.work-exp", "click", function() {
        ...
        $(".my-work-exp").first().clone().insertAfter("div.my-work-exp:last").addClass("currwork");
        $("div.my-work-exp:last .my-title").text("Work Experience "+len);
        $('.currwork input[type="text"]').val(" ");
        $('.currwork input[type="date"]').val(" ");
        $('.currwork select').val(null);
        $('.my-work-exp').removeClass("currwork");
    });

    $("body").delegate("span.add-more-items.edu", "click", function() {
        var len = $('.my-education').length + 1;
        $(".my-education").first().clone().insertAfter("div.my-education:last").addClass("curredu");
        $("div.my-education:last .my-title").text("Education "+len);
        $('.curredu input[type="text"]').val(" ");
        $('.curredu input[type="date"]').val(" ");
        $('.curredu select').val(null);
        $('.my-education').removeClass("curredu");

    });

    $("body").delegate("span.add-more-items.social-med", "click", function() {
        var len = $('.my-social-media').length + 1;
        $(".my-social-media").first().clone().insertAfter("div.my-social-media:last").addClass("cursoc");
        $("div.my-social-media:last .my-title").text("Social Media "+len);
        $('.cursoc input[type="text"]').val(" ");
        $('.my-social-media').removeClass("cursoc");
    ...
    $("body").delegate("span.add-more-items.certi", "click", function() {
        ...
        $(".my-certi").first().clone().insertAfter("div.my-certi:last").addClass("current");
        $("div.my-certi:last .my-title").text("Certifications"+len);
        $('.current input[type="text"]').val(" "); 
        $('.current input[type="date"]').val(" "); 
        $(".my-certi").removeClass("current");       
    });
    ...
    $("body").delegate("span.add-more-items.accomp", "click", function() {
        ...
        $(".my-accomp").first().clone().insertAfter("div.my-accomp:last").addClass("curaccomp");
        $("div.my-accomp:last .my-title").text("Accomplishment"+len);
        $('.curaccomp input[type="text"]').val(" "); 
        $('.curaccomp input[type="date"]').val(" "); 
        $('.curaccomp input[type="hidden"]').val(" ");
        $(".my-accomp").removeClass("curraccomp");
    });
...
<script>
    ...
    $("#career-submit").click(function () {
        ...
        var expsal=$("#exp-salary").val();
        var expcur=$("#exp-currency").val();
        var expsf=$("#exp-sf").val();
        if(expsal.length==0)
        {
            $('#exp-salary-error').text("Please enter your expected salary");
            isValid=false;
            
        }
        if(expcur.length==0)
        {
            $('#exp-currency-error').text("Please enter the salary currency");
            isValid=false;

        }
        if(expsf===null)
        {
            $('#exp-sf-error').text("Please select the salary frequency");
            isValid=false;

        }
        var cursal=$("#current-salary").val();
        var currcur=$("#current-currency").val();
        var cursf=$("#current-sf").val();
        if(cursal.length===""&&currcur.length!="" &&cursf!=" ")
        {
            $('#current-salary-error').text("Please enter the current salary");
            isValid=false;
        }
        if(cursal.length!=="" &&currcur.length==="" &&cursf!==" ")
        {
            $('#current-currency-error').text("Please enter the currency");
            isValid=false;
        }
        if(cursal.length!="" &&currcur.length!="" &&cursf===" ")
        {
            $('#current-sf-error').text("Please enter the current salary frequency");
            isValid=false;
        }
...
if($('input[type=radio][name=fresher]:checked').val() == "no")
{
    ...
    $(".my-education ").each(function()
        ...
        $(".my-accomp").each(function() {
            ...
            var accompnameval=accompname.val();
            ...
            // Check if either name or URL is provided
            if (( accompnameval!== "" && uploadproofval ==="" && startdateval==="" && enddateval==="")) {
                
                uploadproof.next(".error-message").text("Please upload the proof.");
                startdate.next(".error-message").text("Please enter the start date.");
                enddate.next(".error-message").text("Pleaes enter the end date");   
                isValid = false;
            }
            if ((accompnameval===" ")) {
                
                accompname.next(".error-message").text("Please enter the accomplishment name");               
                isValid = false;
            }  
...
//social media
           ...
           if($('#holding-visa').val()=="yes"){
            //visa details
            var visadata=[];
            var index=1;
            $(".visa-details").each(function()
            {
                var visacountry=$(this).find("select[name=visa-country]").val();
                var visatype=$(this).find("input[name=visa-type]").val();
                var visaexpiry=$(this).find("input[name=visa-expiry]").val();

                visadata.push({
                    "Visa details":index,
                    "Visa Country":visacountry,
                    "Visa Type":visatype,
                    "Visa Expriy":visaexpiry
                });
                index++;
            });
...
var WorkExperienceData = [];
            ...
            $(".my-work-exp").each(function() {
                ...
                var role = $(this).find("input[name='role-name']").val();
                var company = $(this).find("input[name='company-name']").val();
...
formData.append("visa",visadetailsJSON);
...
console.log()
    ...
    success: function (response) {
        ...
        if (response === "success") {
            window.scrollTo(0,0);
            $(".success-message").text("Thank you for submitting");
